...
class ViewController: UIViewController, ScanningViewControllerDelegate {
    ...
    @IBAction func upload(_ sender: Any) {
        ...
        let uploadPLYRef = Storage.storage().reference(withPath:"models/\(randomID).ply")
        let uploadJPEGRef = Storage.storage().reference(withPath: "jpeg/\(randomID).jpeg")
        
        let uploadMetaData = StorageMetadata.init()
        uploadMetaData.contentType = "text/plain"
        let uploadJPEGMetaData = StorageMetadata.init()
        uploadJPEGMetaData.contentType = "image/jepg"
        
        // File located on disk
        let localPLYFile = scanPLYURL
        let localJPEGFile = scanThumbnailURL
        
        let uploadPLYTask = uploadPLYRef.putFile(from: localPLYFile, metadata: uploadMetaData) { metadata, error in
        ...
        let uploadTask = uploadJPEGRef.putFile(from: localJPEGFile, metadata: uploadJPEGMetaData) { metadata, error in
            guard let metadata = metadata else {
                // Uh-oh, an error occurred!
                return
            }
